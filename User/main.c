...
float Speed_Left, Speed_Right, Location_Left, Location_Right;
...
int main(void)
{
	...
	while (1)
	{
		...
		if (OLED_Update_Flag)
		{
			...
			OLED_Printf(0, 48, OLED_8X16, "T:%4.0f", Position_Control_Left.Target);
			// OLED_Printf(0, 48, OLED_8X16, "T:%04.0f", CM);

			OLED_Printf(64, 0, OLED_8X16, "LL:%4.2f", Position_Control_Left.Target);
			OLED_Printf(64, 16, OLED_8X16, "LA:%4.2f", Position_Control_Left.Actual);
			OLED_Printf(64, 32, OLED_8X16, "LR:%4.2f", Position_Control_Right.Target);
			OLED_Printf(64, 48, OLED_8X16, "LA:%4.2f", Position_Control_Right.Actual);
			/*********外环位置环******************/
		...
		// Position_Control_Right.Target = RP_GetValue(4) / 4095.0 * 600 - 300;//测试左右位置环
...
void TIM3_IRQHandler(void)
{
	...
	if (TIM_GetITStatus(TIM3, TIM_IT_Update) == SET)
	{
		...
		if (Count2 >= 10) // 中环（速度）
		{
			...
			// TIM1_Motor_SetSpeed(-(int)out_left, -(int)out_right); // 在测试位置环时解除注释
		}
		...
		if (Count3 >= 100) // 外环（位置环）
		{
			...
			// error_Left = Position_Control_Left.Target - distance_Left;
			// error_Right = Position_Control_Right.Target - distance_Right;
