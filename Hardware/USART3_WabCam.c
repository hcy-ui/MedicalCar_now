...
void USART3_Serial_SendArray(uint8_t *Array, uint16_t Length)
{
	...
	{
		USART3_Serial_SendByte(Array[i]);		//依次调用Serial_SendByte发送每个字节数据
	}
}
...
void USART3_Serial_SendString(char *String)
{
	...
	{
		USART3_Serial_SendByte(String[i]);		//依次调用Serial_SendByte发送每个字节数据
	}
}
...
/**
  ...
  * 返 回 值：返回值等于X的Y次方
  */
uint32_t USART3_Serial_Pow(uint32_t X, uint32_t Y)
{
...
void USART3_Serial_SendNumber(uint32_t Number, uint8_t Length)
{
	...
	{
		USART3_Serial_SendByte(Number / USART3_Serial_Pow(10, Length - i - 1) % 10 + '0');	//依次调用Serial_SendByte发送每位数字
	}
}
...
{
	USART3_Serial_SendByte(ch);			//将printf的底层重定向到自己的发送字节函数
	return ch;
}
...
void USART3_Serial_Printf(char *format, ...)
{
	...
	va_end(arg);					//结束变量arg
	USART3_Serial_SendString(String);		//串口发送字符数组（字符串）
}
...
{
	if (USART3_Serial_RxFlag == 1)			//如果标志位为1
	{
